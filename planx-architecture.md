...
## 3. Connector Model (FROZEN)

Planx 4.0 introduces a **three-layer Connector model**:

### Layer 1 — Connector (Repo)

A **Connector** is a logical grouping named after the external system it integrates with:

```
mysql, postgres, kafka, http, file, redis, s3, opensearch, ...
```

Repository naming: `planx-plugin-{connector}`

A connector repo CAN contain multiple plugin binaries:

```
planx-plugin-mysql/
├── cmd/
│   ├── source/main.go     → mysql-source binary
│   └── sink/main.go       → mysql-sink binary
├── internal/
│   ├── source/
│   └── sink/
├── manifests/
...
A connector does NOT have to implement all capabilities. Example:

| Connector | Source | Sink | Processor |
|-----------|--------|------|-----------|
| mysql | ✅ | ✅ | ✗ |
| http | ✗ | ✅ | ✗ |
| transform | ✗ | ✗ | ✅ |
| postgres | ✅ (Alpha) | ✗ (future) | ✗ |

### Layer 2 — Plugin (Runtime)

A **Plugin** is a single executable registry entry. Named `{connector}-{kind}`:
...
Each plugin has its own `manifest.yaml`. The Registry stores plugins individually.

### Layer 3 — Connector Capability (UI)
...
**Designer** and **Marketplace** display Connectors with capability badges.
**Runtime** executes Plugins.

---

## 4. Plugin Discovery Model
